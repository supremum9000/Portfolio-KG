...
import { experienceEntries, experiencePageContent } from '@/data/experienceEntries.js';
import { useLocale } from '@/i18n/LocaleContext';
import { DEFAULT_LOCALE } from '@/i18n/config';

function ExperiencePage() {
  const { locale } = useLocale();
  const content = experiencePageContent[locale] ?? experiencePageContent[DEFAULT_LOCALE];
  const entries = experienceEntries[locale] ?? experienceEntries[DEFAULT_LOCALE];
